...
function EventCountdown({ fechaEvento, nombreEvento = "tu evento" }) {
  ...
  if (esHoy) {
    return (
      <div className="bg-gray-900 rounded-lg p-6 text-white shadow-sm border-2 border-gray-800">
        <div className="flex items-center gap-4">
          <div className="w-16 h-16 bg-white/20 rounded-full flex items-center justify-center">
  ...
  // Countdown normal - Colores profesionales
  const getGradientColor = () => {
    if (diasRestantes <= 7) return 'from-gray-800 to-gray-900';
    if (diasRestantes <= 30) return 'from-gray-700 to-gray-800';
    if (diasRestantes <= 90) return 'from-gray-600 to-gray-700';
