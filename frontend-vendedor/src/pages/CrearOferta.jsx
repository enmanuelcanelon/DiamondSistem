  ...
  */
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Indicador de Progreso - Diseño Premium & Responsive */}
      <div className="mb-8 overflow-x-auto pb-4 -mx-4 px-4 md:mx-0 md:px-0 md:overflow-visible scrollbar-hide">
        <div className="flex min-w-max md:min-w-0 justify-between items-start relative">
          {/* Línea de fondo (solo desktop) */}
          <div className="hidden md:block absolute top-5 left-0 w-full h-0.5 bg-muted -z-10" />

          {nombresPasos.map((nombre, index) => {
            const pasoNumero = index + 1;
            const activo = pasoNumero === pasoActual;
            const completado = pasoNumero < pasoActual || pasoCompleto(pasoNumero);

            return (
              <div
                key={index}
                onClick={() => completado ? irAPaso(pasoNumero) : null}
                className={`flex flex-col items-center relative px-4 md:px-0 group ${completado ? 'cursor-pointer' : ''}`}
              >
                <div
                ...
                <span
                  className={`mt-3 text-xs md:text-sm font-medium transition-colors duration-200 ${activo ? 'text-primary font-bold' : 'text-muted-foreground'
                    }`}
                >
                  {nombre}
                </span>
                ...
                {index < nombresPasos.length - 1 && (
                  <div
                    className={`hidden md:block absolute top-5 left-1/2 w-full h-0.5 -z-0 transition-all duration-500 ${pasoNumero < pasoActual ? 'bg-green-500' : 'bg-transparent'
                      }`}
                  />
                )}
              </div>
                    ...
                    <div className="col-span-1 md:col-span-1">
                      ...
                      <div className="flex gap-2">
                        <select
                          ...
                          className={`flex-1 px-4 py-3 border rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition-colors ${errorHorario ? 'border-red-400 bg-red-50' : (!formData.salon_id || formData.salon_id === '' || !formData.fecha_evento) ? 'border-gray-200 bg-gray-100' : 'border-gray-300 hover:border-gray-400'
                            }`}
                        >
                        ...
                        <select
                          ...
                          className={`w-24 px-3 py-3 border rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition-colors ${errorHorario ? 'border-red-400 bg-red-50' : 'border-gray-300 hover:border-gray-400'
                            }`}
                        >
                    ...
                    <div className="col-span-1 md:col-span-1">
                      ...
                      <div className="flex gap-2">
                        <select
                          ...
                          className={`flex-1 px-4 py-3 border rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition-colors ${errorHorario ? 'border-red-400 bg-red-50' : (!formData.salon_id || formData.salon_id === '' || !formData.fecha_evento || !formData.hora_inicio) ? 'border-gray-200 bg-gray-100' : 'border-gray-300 hover:border-gray-400'
                            }`}
                        >
                          <option value="">Hora</option>
                          {(() => {
                                    ...
                                    <div className="flex items-center justify-between">
                                      {isSelected ? (
                                        ...
                                        <button
                                          ...
                                          className={`w-full flex items-center justify-center gap-2 px-3 py-2 text-sm rounded-lg transition ${tieneExcluyenteSeleccionado
