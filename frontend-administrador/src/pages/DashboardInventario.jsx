                    ...
                    {estaExpandida && (
                      <div className="overflow-x-auto">
                        ...
                        <div className="md:hidden space-y-3 p-3">
                          {itemsCategoria.map((item) => (
                            <div key={item.item_id} className={`p-4 rounded-lg border ${item.necesita_reposicion ? 'bg-red-50 border-red-100' : 'bg-white border-gray-200'} shadow-sm`}>
                              <div className="flex justify-between items-start mb-3">
                                <div>
                                  <h4 className="font-medium text-gray-900">{item.inventario_items?.nombre}</h4>
                                  <p className="text-xs text-gray-500">{item.inventario_items?.unidad_medida}</p>
                                </div>
                                {item.necesita_reposicion ? (
                              ...
                              <div className="grid grid-cols-2 gap-4 mb-4">
                                <div>
                                  ...
                                  {editingItem === item.item_id ? (
                                    ...
                                    <p className="font-medium">{parseFloat(item.cantidad_actual).toFixed(2)}</p>
                                  )}
                                </div>
                                <div>
                                  <p className="text-xs text-gray-500 mb-1">Cantidad Mínima</p>
                                  {editandoCantidadMinima === item.item_id ? (
                                    <div className="flex items-center gap-2">
                                      <input
                                        type="number"
                                    ...
                                    <div className="flex items-center gap-2">
                                      <span className="font-medium">{parseFloat(item.cantidad_minima || 20).toFixed(2)}</span>
                                      <button
                                        onClick={() => {
                                          setEditandoCantidadMinima(item.item_id);
                                          setCantidadMinimaEdit(item.cantidad_minima?.toString() || '20');
                              ...
                              <div className="flex justify-end gap-2 pt-2 border-t border-gray-200/50">
                                {editingItem === item.item_id ? (
                                  ...
                                  <>
                                    ...
                                    <button
                                      onClick={() => setShowTransferencia(item)}
                                      className="p-2 text-purple-600 bg-purple-50 rounded-full"
                                      title="Transferir a salón"
                                    >
                                      <ArrowRightLeft className="w-4 h-4" />
                                    </button>
                            ...
                            return (
                              ...
                              {itemsCategoria.map((item) => {
                                ...
                                return (
                                  <div
                                    key={item.item_id}
                                    className={`p-3 rounded border ${estaSeleccionado ? 'bg-blue-50 border-blue-200' : 'bg-white border-gray-200'}`}
                                  >
                                    <div className="flex items-start gap-3">
                                      ...
                                      <div className="flex-1">
                                        ...
                                        {estaSeleccionado && (
                                          <div className="flex items-center gap-2">
                                            <label className="text-xs text-gray-600">Transferir:</label>
                                            <input
                                              type="number"
                                              value={itemsSeleccionados[item.item_id]?.cantidad || ''}
                                              onChange={(e) => {
                                      ...
                                      <div className="flex items-start gap-3">
                                        ...
                                        <div className="flex-1">
                                          ...
                                          <div className="flex justify-between text-xs text-gray-500 mb-2">
                                            <span className="text-red-600 font-medium">Actual: {parseFloat(item.cantidad_actual).toFixed(2)}</span>
                                            <span>Mín: {parseFloat(item.cantidad_minima || 20).toFixed(2)}</span>
                                            <span>{item.inventario_items?.unidad_medida}</span>
                                          </div>

                                          {estaSeleccionado && (
                                            <div className="flex items-center gap-2">
                                              <label className="text-xs text-gray-600">Comprar:</label>
                                              <input
                                                type="number"
                                                value={itemsSeleccionadosCompra[item.item_id]?.cantidad_a_comprar || ''}
                                                onChange={(e) => {
                        ...
                        <div key={categoria} className="border border-gray-200 rounded overflow-hidden">
                          ...
                          {estaExpandida && (
                            <>
                              ...
                              <div className="md:hidden space-y-2 p-2">
                                {itemsCategoria.map((item) => (
                                  <div key={item.item_id} className="bg-white p-3 rounded border border-gray-200 shadow-sm">
                                    <div className="font-medium text-gray-900 mb-1">{item.nombre}</div>
                                    <div className="flex justify-between text-xs text-gray-500 mb-2">
                                      <span>Solicitado: {parseFloat(item.cantidad_a_comprar || 0).toFixed(2)}</span>
                                      <span>{item.unidad_medida}</span>
                                    </div>

                                    <div className="flex items-center gap-2">
                                      <label className="text-xs text-gray-600">Recibido:</label>
                                      <input
                                        type="number"
                                        value={cantidadesRecibidas[item.item_id] || ''}
                                        onChange={(e) => {
          ...
          <div className="mb-6">
            ...
            <div className="bg-gray-50 p-4 rounded-lg space-y-2">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className={`font-semibold ${confirmarEdicion.diferencia > 0 ? 'text-green-600' :
              ...
              <div className="flex items-center justify-between pt-2 border-t border-gray-200">
                ...
                <span className={`font-bold ${confirmarEdicion.diferencia > 0 ? 'text-green-600' :
                  confirmarEdicion.diferencia < 0 ? 'text-red-600' :
                    'text-gray-900'
                  }`}>
